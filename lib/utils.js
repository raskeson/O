...
// 依照材料清單計算加權碳氮比 (C/N)。
// 每項材料只記錄「碳氮比」（例如 30 代表 30:1），沒有分開的碳%／氮%，
// 所以用「依重量加權的調和平均數」估算：假設各材料對氮池的貢獻大致與重量成正比，
// 總碳氮比 ≈ 總重量 ÷ Σ(重量 ÷ 該材料碳氮比)。這是堆肥估算常用的簡化算法。
export function calcCN(materials) {
  ...
  materials.forEach((m) => {
    ...
    const ratio = Number(m.cn_ratio) || 0;
    if (weight <= 0 || ratio <= 0) return;
    totalWeight += weight;
    weightedInverse += weight / ratio;
  });
  if (weightedInverse === 0) return null;
  return totalWeight / weightedInverse;
}
