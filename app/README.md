...
- 場域天氣的「目前溫度」來自中央氣象署自動氣象站的即時觀測，若該縣市剛好所有測站都故障/離線，會顯示「暫無即時測站觀測資料」，此時仍可參考下方的今明預報區間。

## 這次新增了 2 個資料庫欄位/資料表，記得跑一次 SQL

這次更新在 `supabase/schema.sql` 最下面新增了：
- `sellers` 資料表（賣家清單）
- `plants.death_reason` 欄位（死亡原因）

請到 Supabase 的 **SQL Editor**，把 `supabase/schema.sql` 全部內容**重新貼上執行一次**（已經建立過的資料表/欄位會自動跳過，不會出錯或清空舊資料）。

## CSV 匯入功能

植物圖鑑頁、資材頁都新增了「📥 匯入 CSV」按鈕：
1. 按「📄 下載空白範本 CSV」拿到正確的欄位名稱
2. 用 Excel 打開範本、填資料、另存成 CSV（Excel 選「CSV UTF-8 (逗號分隔)」格式）
3. 回到網站按「匯入 CSV」上傳剛剛存的檔案，預覽筆數沒問題後按「確認匯入」

植物 CSV 的 `field_name` 欄位填的是場域「名稱」（不是 ID），系統會自動比對；比對不到就會匯入成「未設定場域」，之後可以用「搬家」功能手動指定。`cost` 有值的話會自動記一筆購入成本到財務總帳；資材 CSV 的 `qty`×`price` 有值的話會自動記一筆支出。
